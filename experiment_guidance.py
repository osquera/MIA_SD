...
experiment1 = ["0", "DTU_vs_AAU_unseen_test"]
experiment2 = ["4", "DTU_vs_AAU_unseen_test"]
experiment3 = ["8", "DTU_vs_AAU_unseen_test"]
experiment4 = ["12", "DTU_vs_AAU_unseen_test"]
experiment5 = ["16", "DTU_vs_AAU_unseen_test"]


experiment_set = [experiment1, experiment2, experiment3, experiment4, experiment5]

num_experiments = len(experiment_set)

root_im = cwd + os.sep + 'images_attack_model' + os.sep + 'guidance_experiment' + os.sep
...
def run_experiment(root_data, root_data_test, experiment_set):
    for i in range(num_experiments):
        ...
        for seed in range(REPEAT_EXPERIMENT):
            ...
            if os.path.exists(f'{root_data[i]}/{experiment_set[i][0]}model_{seed}.pt'):
                model.load_state_dict(torch.load(f'{root_data[i]}{experiment_set[i][0]}model_{seed}.pt'))
                model.to(device)
                model.eval()
                ...
                # Save the model
                torch.save(model.state_dict(), f'{root_data[i]}{experiment_set[i][0]}model_{seed}.pt')
        ...
        plt.savefig(f'{root_data_test[i]}{experiment_set[i][0]}_ROC.png')
        plt.savefig(f'{root_im}figures/{experiment_set[i][0]}_ROC.pgf')
        plt.close()
        ...
        plt.savefig(f'{root_data_test[i]}{experiment_set[i][0]}_CM.png')
        plt.savefig(f'{root_im}figures/{experiment_set[i][0]}_CM.pgf')
        plt.close()

        # Save the results to a pickle file
        results = {'roc_auc': roc_auc, 'tpr': tpr, 'fpr': fpr, 'thresholds': thresholds, 'y_pred': y_pred, 'y_true': y_true, 'cm': cm}
        pd.to_pickle(results, f'{root_data_test[i]}{experiment_set[i][0]}results.pkl')
...
def create_plots(root_data_test, experiment_set):
    for i in range(len(experiment_set)):
        results = pd.read_pickle(f'{root_data_test[i]}{experiment_set[i][0]}results.pkl')
